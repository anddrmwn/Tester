...
function filterByDate($koneksi, $start, $end, $user_id)
{
    ...
    $query = "SELECT * FROM `tb_pajak` WHERE `user_id` = '$user_id' AND `tanggal` BETWEEN '$start_datetime' AND '$end_datetime' ORDER BY `tanggal` DESC";
...
if ($start && $end) {
    ...
    $result = mysqli_query($koneksi, "SELECT * FROM `tb_pajak` WHERE `user_id` = '$user_id' ORDER BY `tanggal` DESC");
}
                            ...
                            <div class="card mb-4">
                                <div class="card-header">
                                    <form action="" method="get" class="form-inline">
                                        ...
                                        <button type="button" class="btn btn-secondary btn-sm my-2" onclick="window.location.href='<?= $base_url ?>pajak'">Reset</button>
                                    </form>
                                </div>
                            </div>
                        </div>
                        <div class="col-lg-12">
                            <div class="card mb-4">
                                <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                                    ...
                                    <button type="button" class="btn btn-primary btn-sm" onclick="window.location.href='<?= $base_url ?>pajak/create.php'">Add Data</button>
                                </div>
                                        ...
                                        <tbody>
                                            ...
                                            ?>
                                                <tr>
                                                    ...
                                                    <td><?= $row['deskripsi'] ?></td>
                                                    <td><?= $row['tipe'] ?></td>
                                                    <td><a href="<?= $base_url ?>pajak/edit.php?id=<?= $row['pajak_id'] ?>">Edit</a> | <a href="<?= $base_url ?>pajak/delete.php?id=<?= $row['pajak_id'] ?>" onclick="return confirm('Are you sure you want to delete this data?')">Delete</a></td>
                                                </tr>
                                            <?php
                                            endforeach;
                                            ?>
